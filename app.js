...
app.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    res.json({
      downloadUrl: `https://file2do.com/compressed/${outputFilename}`,
      size: compressedSize,
    });
...
app.post('/compress-pdf', upload.single('file'), async (req, res) => {
  ...
  if (compression >= 90) {
    ...
    try {
      ...
      res.json({
        downloadUrl: `https://file2do.com/compressed/${path.basename(outputPdf)}`,
        originalSize,
        finalSize,
...
app.post('/merge-pdf', upload.array('files'), async (req, res) => {
  ...
  try {
    ...
    res.json({ downloadUrl: `https://file2do.com/compressed/${outputFilename}` });
...
app.post('/pdf-to-word', upload.single('file'), async (req, res) => {
  ...
  exec(sofficeCmd, async (err, stdout, stderr) => {
    ...
    glob(`${outputDir}/*.docx`, (globErr, files) => {
      ...
      res.json({
        downloadUrl: `https://file2do.com/compressed/${path.basename(latestFile)}`,
        size,
      });
    });
  });
...
app.post('/pdf-to-jpg', upload.single('file'), (req, res) => {
  ...
  exec(cmd, async (err, stdout, stderr) => {
    ...
    output.on('close', () => {
      res.json({
        downloadUrl: `https://file2do.com/compressed/${zipName}`,
